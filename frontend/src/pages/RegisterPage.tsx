...
export default function RegisterPage() {
  ...
  const barColors = ['#27272a', '#ef4444', '#f59e0b', '#10b981', '#10b981'];
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: '#09090b' }}>
      <div style={{ width: '100%', maxWidth: 340 }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 700, color: '#fafafa', letterSpacing: '-0.025em', marginBottom: 4 }}>Create account</h1>
        <p style={{ fontSize: 13.5, color: '#71717a', marginBottom: 28 }}>Start simulating policy outcomes in minutes.</p>
        ...
        <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ position: 'relative' }}>
            <User style={{ position: 'absolute', left: 11, top: '50%', transform: 'translateY(-50%)', width: 15, height: 15, color: '#52525b', pointerEvents: 'none' }} />
            <input type="text" placeholder="Full name" value={name} onChange={(e) => setName(e.target.value)} required className="input input-icon" />
          </div>
          <div style={{ position: 'relative' }}>
          ...
          {password.length > 0 && (
            <div style={{ display: 'flex', gap: 4 }}>
              {[1,2,3,4].map(i => (
                <div key={i} style={{ flex: 1, height: 2, borderRadius: 2, background: i <= strength ? barColors[strength] : '#27272a', transition: 'background 0.2s' }} />
              ))}
            </div>
          )}

          <button type="submit" disabled={loading} className="btn btn-solid" style={{ marginTop: 4 }}>
            {loading ? <><span style={{ width:14,height:14,borderRadius:'50%',border:'2px solid rgba(255,255,255,0.3)',borderTopColor:'white',display:'inline-block',animation:'spin 0.7s linear infinite' }}/>Creating…</> : <>Create account <ArrowRight size={14}/></>}
          </button>
        </form>
        ...
        <p style={{ textAlign: 'center', fontSize: 11.5, color: '#3f3f46', marginTop: 12, lineHeight: 1.6 }}>
